...
void row_rms_norm_f32_dim_cpu(float* in, float* weight, float* out, int batch,
                              int size, float eps) {
...
__global__ void row_rms_norm_f32_dim_simd(float* in, float* weight, float* out,
...
__global__ void row_rmsnorm_d32_sim(float* in, float* wei, float* out,
                                    int batch, int size, float eps) {
  const int bid = blockIdx.x;
  if (bid >= batch) return;
...
int main() {
  ...
  row_rms_norm_f32_dim_cpu(h_input.data(), h_weight.data(), h_output_cpu.data(),
                           batch, size, eps);
  auto end = std::chrono::high_resolution_clock::now();
  auto duration =
      std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
  std::cout << "CPU RMSNorm took" << duration.count() << " milliseconds.\n";
  ...
  for (int i = 0; i < warpup; i++) {
    // warmup run
    row_rmsnorm_d32_sim<<<grid, block>>>(d_input, d_weight, d_output, batch,
                                         size, eps);
  }
